...
# MultiOutputRegressor の代わりに、自前ループで各出力を学習
estimators = []
...
# multi_lgbm.predict はスケール済みデータを返すので…
y_pred_lgbm_scaled = multi_lgbm.predict(X_te)
y_pred_lgbm        = scaler.inverse_transform(y_pred_lgbm_scaled)
...
class DoubleGauss(Optic):
    ...
    def reset(self):
        super()._initialize_attributes()  # 属性だけ初期化。これで通る。
...
#    def randomly_misalign(self, delta_pos_max=0.3):
#        delta_pos = np.random.uniform(-delta_pos_max, delta_pos_max, size=(4, 3))
#        for i, (dx, dy, dz) in enumerate(delta_pos):
#            self.decenter(i, dx, dy, dz)         
#        return delta_pos.ravel().reshape(1, -1)        # shape (1, 12)
...
# flatten() して長さ 20 のベクトル同士を scatter
