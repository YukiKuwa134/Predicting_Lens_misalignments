...
class ZernikeMisalignmentDataset(Dataset):
    def __init__(self, X_path, y_path, transform=None):
        self.X = np.load(X_path).astype(np.float32)   # shape = (N, n_feats)
        self.y = np.load(y_path).astype(np.float32)   # shape = (N, 20)
...
# 2) MLP モデルを定義 Residual（ショートカット）接続
...
def main():
    ...
    lr = 1e-3
    max_epochs = 100
    patience = 40  # EarlyStopping のエポック数
    ...
    y = np.load("DoubleGauss_misalignments.npy")   # shape=(N,20)
...
#---- 推論に切り替え ----
    ...
    # (1) テストデータを Tensor／NumPy に準備
    X_test_np = ds.X
    y_test_np = np.load("DoubleGauss_misalignments.npy")

    X_test = torch.from_numpy(X_test_np).float().to(device)
    y_test = y_test_np  # NumPy array (N, 20)
    ...
    # (4) mm系／deg系 に分けた RMSE
    label_list = []
    for i in range(4):         # 4 エレメント
        num = i+1
        for ax in ("dx","dy","dz","tip","tilt"):
    ...
    # ② 比較したいサンプルのインデックス
    idx = 0   # ここを他の番号に変えれば好きなサンプルを表示できます
